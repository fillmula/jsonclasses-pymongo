...
class TestMongoObjectQuery(unittest.TestCase):
  ...
  def test_include_includes_many_to_many(self):
    ...
    class UestAuthor(MongoObject):
      name: str
      posts: List[UestPost] = types.listof('UestPost').linkedthru('authors')
    @jsonclass
    class UestPost(MongoObject):
      title: str
      authors: List[UestAuthor] = types.listof('UestAuthor').linkedthru('posts')
    author = UestAuthor(**{ 'name': 'Michael', 'posts': [{ 'title': 'PA' }, { 'title': 'PB' }] })
    author.save()
